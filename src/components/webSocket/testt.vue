<template>
  <div class="websocket">
    <el-button type="primary" class="chatBtn" @click="mydialogTableVisible = true"
      >聊天窗口</el-button
    >
    <el-dialog
      title=""
...
export default {
  ...
  methods: {
    ...
    handlePreview(file) {
      ...
      this.myCYYfile = file.raw;
      var fileSize = this.myCYYfile.size; //文件大小
      var filetType = this.myCYYfile.type; //文件类型
      //创建文件读取对象
    ...
    async sendBtn(e) {
      ...
      if (
        document.getElementsByClassName("el-upload-list__item-name").length !=
          0 &&
        ...
        await this.$api.ossststoken().then((res) => {
